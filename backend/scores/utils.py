...
def _calculate_combined_metrics(student: Student, year: int, period: str, total_score: float) -> dict:
    ...
    grade_highest = max(grade_scores) if grade_scores else 0
    ...
    if school_id:
        ...
        if school_total:
            ...
            school_highest = max(school_scores)
    ...
    national_highest = max(national_scores) if national_scores else 0
    ...
    return {
        ...
        'national_highest': national_highest,
...
def _collect_trend_data(student: Student, grade_level: str) -> dict:
    ...
    # デバッグログ
    logger.info(f"[GRAPH_DEBUG] Trend data for student {student.student_id} ({student.name}):")
    logger.info(f"[GRAPH_DEBUG]   TestResult count: {results_qs.count()}")
    logger.info(f"[GRAPH_DEBUG]   Timeline keys: {sorted_keys}")
    logger.info(f"[GRAPH_DEBUG]   Overall trend ({len(overall_trend)} items): {overall_trend}")
    logger.info(f"[GRAPH_DEBUG]   Subject trend: {subject_trend}")
    
    if not overall_trend:
        logger.warning(f"[GRAPH_DEBUG] WARNING: No trend data found for student {student.student_id}")
...
def _generate_svg_bar_chart(scores: list[float], avg_scores: list[float], color: str, chart_id: str, max_score: float = 100) -> str:
    """成績推移の棒グラフをSVGで生成"""
    # デバッグ情報をログ出力
    import logging
    logger = logging.getLogger(__name__)
    logger.info(f"[GRAPH_DEBUG] _generate_svg_bar_chart called: chart_id={chart_id}, scores={scores}, avg_scores={avg_scores}")
    
    if not scores:
        # データがない場合でも、メッセージ付きSVGを返す
        return '''<svg viewBox="0 0 200 140" class="line-chart" id="{}" xmlns="http://www.w3.org/2000/svg">
            <text x="100" y="70" text-anchor="middle" font-size="12" fill="#666">データなし</text>
        </svg>'''.format(chart_id)

    half_score = int(max_score / 2)
    ...
    svg_parts = [
        ...
        '<!-- Y軸ラベル -->',
        f'<text x="20" y="20" font-size="9" fill="#666">{max_score_int}</text>',
        f'<text x="20" y="65" font-size="9" fill="#666">{half_score}</text>',
    ...
    # 学年平均の星印とライン
    # avg_scoresが単一の値の場合はリストに変換（後方互換性のため）
    if isinstance(avg_scores, (int, float)):
        avg_scores = [avg_scores] * len(scores)
    
    # データ数が合わない場合は調整
    if len(avg_scores) < len(scores):
        avg_scores = avg_scores + [0] * (len(scores) - len(avg_scores))

    for index in range(len(scores)):
        x = 60 + (index * spacing)
        avg_score = avg_scores[index]
        ...
        if index > 0:
            ...
            prev_avg = avg_scores[index - 1]
            prev_avg_y = 110 - ((prev_avg - min_score) / (max_score - min_score)) * 90
            
            svg_parts.insert(11,  # グリッド線の後に挿入
                f'<line x1="{prev_x}" y1="{prev_avg_y}" x2="{x}" y2="{avg_y}" '
...
def _prepare_template_data(report_data: dict, logo_path: str) -> dict:
    ...
    math_data = subjects.get('math', {})
    japanese_data = subjects.get('japanese', {})

    # 問題別データ
    math_questions = []
    for q in math_data.get('question_details', []):
    ...
    japanese_questions = []
    for q in japanese_data.get('question_details', []):
    ...
    # 学年平均を計算（各回の平均のリスト）
    total_avgs = [t.get('average', 0) for t in overall_trends]
    math_avgs = [t.get('average', 0) for t in math_trends]
    japanese_avgs = [t.get('average', 0) for t in japanese_trends]

    # デバッグログ
    import logging
    logger = logging.getLogger(__name__)
    logger.info(f"[GRAPH_DEBUG] Preparing graph for student {student_info['id']} ({student_info['name']}):")
    logger.info(f"[GRAPH_DEBUG]   Total scores: {total_scores}, avgs: {total_avgs}")
    logger.info(f"[GRAPH_DEBUG]   Math scores: {math_scores}, avgs: {math_avgs}")
    logger.info(f"[GRAPH_DEBUG]   Japanese scores: {japanese_scores}, avgs: {japanese_avgs}")

    # 各教科のSVGグラフを生成
    total_max_score = combined.get('max_score', 200)
    math_max_score = math_data.get('max_score', 100)
    japanese_max_score = japanese_data.get('max_score', 100)

    total_chart_svg = _generate_svg_bar_chart(total_scores, total_avgs, '#3498db', f'totalChart_{student_info["id"]}', total_max_score)
    math_chart_svg = _generate_svg_bar_chart(math_scores, math_avgs, '#27ae60', f'mathChart_{student_info["id"]}', math_max_score)
    japanese_chart_svg = _generate_svg_bar_chart(japanese_scores, japanese_avgs, '#e67e22', f'japaneseChart_{student_info["id"]}', japanese_max_score)
    ...
    return {
        ...
        'japanese_score': japanese_data.get('total_score', 0),
        'total_score': combined.get('total_score', 0),

        'math_deviation': f"{math_data.get('deviation', 0):.1f}" if math_data.get('deviation') is not None else '-',
        ...
        'principal_math_comment': _get_principal_comment(student_info['id'], test_info['year'], test_info['period'], 'math'),
...
def create_individual_report_pdf(report_data: dict) -> tuple[str | None, str | None]:
    """個人成績表PDF生成（HTML/CSS版 - WeasyPrint使用）"""
    try:
