...
interface TestDownloadClientProps {
  year: string;
  period: string;
}

export function TestDownloadClient({ year, period }: TestDownloadClientProps) {
  ...
  const handleYearChange = (newYear: string) => {
    ...
    router.push(`/tests/${newYear}/${selectedPeriod}/download`);
  };

  const handlePeriodChange = (newPeriod: string) => {
    setSelectedPeriod(newPeriod);
    router.push(`/tests/${selectedYear}/${newPeriod}/download`);
  };
  ...
  const { data: testFiles } = useQuery({
    ...
    queryFn: async () => {
      try {
        // 実際のAPIエンドポイントに修正（バックエンドのテストファイルAPIを想定）
