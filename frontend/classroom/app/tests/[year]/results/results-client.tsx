...
export function ResultsClient({ year }: ResultsClientProps) {
  ...
  const period = searchParams.get('period') || 'summer'; // URLパラメータから期間を取得、デフォルトは夏期
  ...
  const { 
    ...
    queryKey: ['testResults', year, period, subject],
    queryFn: () => testApi.getTestResults({ year, period, subject: subject || undefined }),
    enabled: !!year && !!period
  });
  ...
  const { 
    ...
    queryKey: ['testStatistics', year, period, subject],
    queryFn: () => testApi.getTestStatistics({ year, period, subject: subject || undefined }),
    enabled: !!year && !!period
  });
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
              {year}年度 {({ spring: '春期', summer: '夏期', winter: '冬期' }[period] || period)}テスト結果
            </h1>
