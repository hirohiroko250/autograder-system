...
function TestResultsContent({ year, period }: { year: string; period: string }) {
  ...
  const getScoreInputStatus = (student: any) => {
    // 欠席判定を最初に確認
    if (student.combined_results && student.combined_results.attendance === false) {
      return 'absent'; // 欠席
    }
    
    // subject_resultsがある場合も欠席判定
    if (student.subject_results) {
      const hasAbsentRecord = Object.values(student.subject_results).some((subjectData: any) => 
        subjectData.attendance === false
      );
      if (hasAbsentRecord) {
        return 'absent'; // 欠席
      }
    ...
    // subject_resultsで点数がある場合
    if (student.subject_results) {
      const hasScore = Object.values(student.subject_results).some((subjectData: any) => 
        subjectData.total_score > 0
      );
      if (hasScore) {
        return 'complete'; // 完了
      }
    ...
    if (studentSearchQuery) {
      ...
      filtered = filtered.filter((result: any) => {
        return (
          ...
          (result.classroom_name && result.classroom_name.toLowerCase().includes(query))
        );
      });
    }
          ...
          <div className="space-y-3">
            {filteredResults.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-sm">
                  {studentSearchQuery ? '検索条件を変更してください' : 'フィルター条件を変更してください'}
                </p>
              </div>
            ) : (
