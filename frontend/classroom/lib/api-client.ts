...
export const testApi = {
  ...
  getTestResults: async (params: { year: string; period: string; subject?: string }): Promise<ApiResponse<TestResult>> => {
    const requestParams = { ...params, app_context: 'classroom' };
  ...
  getIntegratedTestResults: async (params: { year: string; period: string; school?: string; classroom?: string }): Promise<ApiResponse<any>> => {
    const requestParams = { ...params, app_context: 'classroom' };
...
export const commentApi = {
  ...
  getCommentTemplatesV2: async (params?: { category?: string; comment_type?: string; is_active?: boolean }): Promise<ApiResponse<any>> => {
    const response = await apiClient.get<ApiResponse<any>>('/comment-templates-v2/', { params });
    return response.data;
  },
  ...
  createCommentTemplateV2: async (data: {
    ...
    const response = await apiClient.post<any>('/comment-templates-v2/', data);
    return response.data;
  },
  ...
  updateCommentTemplateV2: async (id: string, data: {
    ...
    const response = await apiClient.patch<any>(`/comment-templates-v2/${id}/`, data);
    return response.data;
  },
